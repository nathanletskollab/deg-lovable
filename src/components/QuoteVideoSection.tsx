    ...
    >
      ...
      <div className="relative w-full overflow-hidden pt-16 md:pt-40 pb-10 md:pb-24 px-6 md:px-12">
        <div className="relative z-10 max-w-5xl mx-auto">
          {/* Opening quote mark */}
          <motion.span
            className="block leading-none mb-6 md:mb-8"
          ...
          {/* Quote text — staggered by phrase */}
          <blockquote className="type-display font-normal leading-[1.2] tracking-tight">
            <motion.span
              className="italic"
              style={{ color: 'rgba(255,255,255,0.55)', display: 'inline' }}
              initial={{ opacity: 0, transform: 'translateY(24px)', filter: 'blur(8px)' }}
              whileInView={{ opacity: 1, transform: 'translateY(0px)', filter: 'blur(0px)' }}
              viewport={{ once: true }}
              transition={{ duration: 1.2, ease: [0.23, 1, 0.32, 1], delay: 0.15 }}
            >
              Health lives in balance
            </motion.span>
            <motion.span
              style={{ color: 'rgba(255,255,255,0.80)', display: 'inline' }}
              initial={{ opacity: 0, transform: 'translateY(24px)', filter: 'blur(8px)' }}
              whileInView={{ opacity: 1, transform: 'translateY(0px)', filter: 'blur(0px)' }}
              viewport={{ once: true }}
              transition={{ duration: 1.2, ease: [0.23, 1, 0.32, 1], delay: 0.3 }}
            >
              —when your body is at ease, your energy flows, and you're free to be present for what matters.
            </motion.span>
          </blockquote>
        </div>
      </div>
